...
public class MasterMind {
	...
	private static final String iniciarcolor = "Escribe la inicial del color número ";
...
pedirJugada (i);
	...
	private long visualizarsalida(boolean acertada, int i, String nuevoParametro) {
		System.out.println ();
		if (acertada) {
			quebien = "¡ENHORABUENA! Has acertado la combinación en ";
			System.out.println (quebien + (i-1) +" intentos en la partida nº " + numPartidas);
		} else {
			String losentimos = "LO SENTIMOS. No has conseguido acertar la combinación en la partida nº ";
	...
	private void pedirJugada (int j) throws IOException {
		int i;
		for (i=0; i<=4; i++) {
			System.out.println (iniciarcolor + (i+1) +": ");
	...
	private long modificarheridos(int numRM, int numVM, int numAM, int numBM, int numNM, int numMM, int numGM, int numSM,
			...
			int numMJugada, int numGJugada, int numSJugada, long heridos) {
